...
int main()
{
    ...
    {   
        ...
        double *C_cpu = (double*)malloc(N * P * sizeof(double)); // For verification

        // @@ 2. Allocate in device memory.
        double *A_gpu = nullptr;
        double *B_gpu = nullptr;
        double *C_gpu = nullptr;
        CHECK(cudaMalloc(&A_gpu, N * M * sizeof(double)));
        CHECK(cudaMalloc(&B_gpu, M * P * sizeof(double)));
        CHECK(cudaMalloc(&C_gpu, N * P * sizeof(double)));
